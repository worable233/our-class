...
import { Plus, Download, File as FileIcon } from '@lucide/vue'
...
const courseList = computed(() => {
  if (!selectedClass.value) return allCourses.value
  return allCourses.value.filter(c => c.class === selectedClass.value)
})
...
async function loadClasses() {
  try { allClasses.value = await api.get<ClassInfo[]>('/classes') } catch {}
}
async function load() {
  loading.value = true
  ...
  loading.value = false
  if (!selectedCourse.value && crs.length > 0) selectedCourse.value = crs[0].id
  ...
  <div>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
      <div>
        ...
        <NText depth="3" style="display:block;margin:0;font-size:14px;">按班级和课程查看作业及学生提交情况</NText>
      </div>
    </div>

    <!-- Toolbar: Class + Course Selectors -->
    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:20px">
      <n-select
        v-model:value="selectedClass"
        :options="classList.map(c => ({ label: c.name, value: c.name }))"
        placeholder="选择班级"
        clearable
        style="width:160px"
        size="small"
        @update:value="selectedCourse = null"
      />
      <n-select
