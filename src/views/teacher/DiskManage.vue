...
function toggleSelectAll() {
  if (selectedPaths.value.size === entries.value.length) selectedPaths.value = new Set()
  else selectedPaths.value = new Set(entries.value.map(e => e.path))
}
...
function confirmDelete(entry: FileEntry) {
  dialog.warning({
    title: '确认删除', content: `确定删除「${entry.name}」${entry.is_dir ? '及其所有内容' : ''}？`,
...
function batchDelete() {
  ...
  dialog.warning({
    title: '批量删除', content: `确定删除选中的 ${paths.length} 个项目？`,
    positiveText: '删除', negativeText: '取消',
    onPositiveClick: async () => {
      try { const r = await api.post<{ deleted: number }>('/storage/batch-delete', { paths }); message.success(`已删除 ${r.deleted} 个项目`); selectedPaths.value = new Set(); await loadList(currentPath.value); await loadInfo() }
      catch (e: any) { message.error(e.message || '删除失败') }
    },
  })
}
...
async function loadMoveDirs() {
  try {
    const res = await api.get<{ entries: FileEntry[] }>(`/storage/list?path=${encodeURIComponent(moveDir.value)}`)
    moveDirs.value = res.entries.filter(e => e.is_dir).map(e => e.name)
  } catch { moveDirs.value = [] }
}

function enterMoveDir(name: string) {
  moveDir.value = moveDir.value ? `${moveDir.value}/${name}` : name
  loadMoveDirs()
}

async function confirmMove() {
  try {
    const r = await api.post<{ moved: number }>('/storage/move', { paths: moveTargets.value, target: moveDir.value })
    message.success(`已移动 ${r.moved} 个项目`)
    showMoveModal.value = false; selectedPaths.value = new Set()
    await loadList(currentPath.value); await loadInfo()
...
function iconColor(icon: string): string {
  const map: Record<string, string> = { folder: '#f0a020', image: '#18a058', video: '#a050dc', audio: '#5E6AD2', pdf: '#d03050', doc: '#5E6AD2', excel: '#18a058', ppt: '#d03050', archive: '#f0a020', text: '#888', code: '#5E6AD2' }
  return map[icon] || '#888'
}
...
<template>
  <div style="display:flex;flex-direction:column;gap:16px;">
    ...
    <!-- ═══ 文件列表（行视图） ═══ -->
    <n-card :bordered="true" size="small" style="flex:1;min-height:300px;padding:0;overflow:hidden;"
        ...
        <template v-if="entries.length > 0">
          ...
          <div class="list-header" @click="toggleSelectAll">
            <div style="width:36px;display:flex;align-items:center;justify-content:center;">
              <NCheckbox :checked="selectedPaths.size === entries.length && entries.length > 0" />
            </div>
            <div class="col-icon" style="width:32px" />
            <div class="col-name" style="flex:1;min-width:0;">名称</div>
            <div class="col-size" style="width:80px;text-align:right;">大小</div>
            <div class="col-date" style="width:130px;">修改日期</div>
            <div class="col-actions" style="width:80px;text-align:right;">操作</div>
          </div>
          <!-- 行 -->
          <div v-for="entry in entries" :key="entry.path"
            class="list-row"
            :class="{ selected: selectedFile?.path === entry.path, 'row-selected': selectedPaths.has(entry.path) }"
            @click="selectedFile = entry"
            @dblclick="enterDir(entry)"
          >
            <div style="width:36px;display:flex;align-items:center;justify-content:center;" @click.stop="toggleSelect(entry.path)">
              <NCheckbox :checked="selectedPaths.has(entry.path)" />
            </div>
            <div class="col-icon" style="width:32px;" @click.stop="enterDir(entry)">
              <component :is="fileIconComponent(entry.icon)" :size="20" :stroke-width="1.5" :style="{color:iconColor(entry.icon)}" />
            </div>
            <div class="col-name" style="flex:1;min-width:0;font-weight:500;" @click.stop="enterDir(entry)">
              <span style="cursor:pointer;">{{ entry.name }}</span>
            </div>
            <div class="col-size" style="width:80px;text-align:right;color:var(--text-muted);font-size:12px;">
              <span v-if="!entry.is_dir">{{ entry.size_display }}</span>
              <span v-else style="color:var(--text-muted);font-size:12px;">文件夹</span>
            </div>
    ...
    <n-modal v-model:show="showMoveModal" preset="card" title="移动到" style="width:420px;" :mask-closable="false">
      <div style="margin-bottom:12px;">
        <span style="font-size:13px;color:var(--text-muted);">移动 {{ moveTargets.length }} 个项目到：</span>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:12px;">
        <NTag size="small" :bordered="false" round closable @close="moveDir = ''" style="cursor:pointer;">根目录</NTag>
        <template v-if="moveDir">
          <span style="color:var(--text-muted);font-size:12px;line-height:24px;">/</span>
          <span style="font-size:13px;color:var(--text-primary);font-weight:500;line-height:24px;">{{ moveDir }}</span>
        </template>
      </div>
      <div style="max-height:240px;overflow-y:auto;display:flex;flex-direction:column;gap:2px;">
        <div v-for="name in moveDirs" :key="name"
          style="display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:4px;cursor:pointer;font-size:13px;"
          @click="enterMoveDir(name)"
        ...
        >
          <Folder :size="16" style="color:#f0a020;flex-shrink:0;" />
          <span style="color:var(--text-primary);">{{ name }}</span>
        </div>
        <div v-if="moveDirs.length === 0" style="color:var(--text-muted);font-size:12px;padding:12px;text-align:center;">
          {{ moveDir ? '此文件夹为空' : '根目录下无文件夹' }}
        </div>
      </div>
      <template #footer>
        <NSpace justify="end">
